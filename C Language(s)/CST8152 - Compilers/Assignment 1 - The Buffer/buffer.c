...
/************************************************************************************************
  ...
  Called Functions: 1. calloc()
        ...
        Parameters: short init_capacity - If value exceeds SHRT_MAX or under 0 it will return 
					    ...
					    char inc_factor - Can accept any value and is the amount to increase
...
Buffer *b_allocate(short init_capacity, char inc_factor, char o_mode) {
	...
	/* These 2 ifs ensure the paramters being passed in are valid in the specific mode. */
	...
	if (o_mode == 'f'|| inc_factor == 0) {
			if ((unsigned char)inc_factor == 0 || (unsigned char)inc_factor != 0) {
				buff->mode = 0;
				buff->inc_factor = 0;
				o_mode = 0;
			}
	...
	/*If o_mode is in multiplicative mode it sets the 'mode' to -1 and 'inc_factor' to the
	  inc_factor parameter.*/
		if (o_mode == 'm') {
			if (inc_factor < 0 || inc_factor > 100) {
				return NULL;
			}
			else {
				buff->mode = -1;
...
Called Functions: 1. realloc()
    ...
    Return Value: - Returns a pointer to a initialized Buffer structure on success.'
       ...
       Algorithm: 1. Ensure parameters input are valid
                  2. Allocate memory for a Buffer object and character array
                  3. Initialize Buffer object variables and point it's 'cb_head' to the
				     dynamically alllocated character array.
                  4. Return the initialized Buffer object
************************************************************************************************/
pBuffer b_addc(pBuffer const pBD, char symbol) {

	/*If the Buffer object is full, return NULL.*/
...
char b_getc(Buffer * const pBD) {
	...
	/*If the Buffer ovject passed in is NULL return -2*/
	if (pBD == NULL) {
		return -2;
	}
...
Called Functions: none
    ...
    Return Value:             0 ~ Returned if the parameter is NULL
       ...
       Algorithm: 1. Ensure parameters input are valid.
                  2. Returns the getc_offset plus 1.
************************************************************************************************/
short b_getcoffset(Buffer* const pBD) {
	...
	/*Returns getc_offset and increments it.*/
	return 	pBD->getc_offset++;
}
